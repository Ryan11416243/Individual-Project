...
### 1. Data Generation 
The simulation script generates transformer FRA traces based on established electromagnetic RLC lumped-parameter models. 
...
* **Operational Variance and Noise:** Applies operational variance (thermal drift) and physical measurement noise mimicking Vector Network Analyzer (VNA) instrumentation to ensure realistic datasets.
...
The extraction script transforms raw simulation traces into an ML-ready tabular dataset.
...
* **Diagnostic Visualization:** Automatically generates analytical plots, including feature ablation studies, data scalability matrices (transformer population width vs. historical depth), normalized confusion matrices, and t-SNE high-dimensional manifolds.
...
1. **Generate the Transformer Population Data:** Run the simulation engine to generate the raw text traces.
   `python multi_gen.py`
